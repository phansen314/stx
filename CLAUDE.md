...
A local, **loopback-only task daemon** with a Python front end. Work is organized
**workspace → track → segment → task**; a `blocks` DAG drives a `next`/ready frontier.
The daemon is the **sole writer**; clients (TUI, CLI, sim) speak JSON over HTTP on
`127.0.0.1:8420`.
...
| `stxc/`   | Shared Python wire client (used by CLI + TUI + sim)         |
| `cli/`    | `stx` CLI (`bin/stx`, `python3 -m cli`) — see `docs/stx-cli.md` |
| `tui/`    | Python Textual TUI (`python3 -m tui`)                        |
| `bin/stx` | Bash launcher for the CLI (works from any CWD)              |
...
- Python side: `pip install -r tui/requirements.txt`; `python3 -m tui`; `pytest` (config
  in `pyproject.toml`); `python3 scripts/dev_sim.py` (creates + archives its own workspace).
...
Kotlin 2.3 / JDK 21, Gradle (Kotlin DSL, committed wrapper), http4k (SunHttp loopback),
kotlinx.serialization, kotlinx.coroutines, plain JDBC + `xerial/sqlite-jdbc`,
