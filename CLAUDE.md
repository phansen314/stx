...
- Group subcommands: `stx group create|ls|show|rename|edit|archive|mv|assign|unassign|edge`
- Workspace subcommands: `stx workspace create|ls|show|use|rename|archive`
- Edge subcommands: `stx edge create|archive|ls` and `stx edge meta ls|get|set|del` — task edges carry a `--kind` label and a per-edge metadata blob. `stx group edge …` mirrors the task surface; `create`/`archive`/`meta *` take split `--source-project`/`--target-project` flags to disambiguate when cross-project. `stx group edge ls` uses `--source-project` (not `--project`) to disambiguate the `--source` filter — matches `create`/`archive`/`meta *`. Self-loops are rejected by DB CHECK; multi-hop cycles are currently allowed (cycle detection deferred pending blocking-kind semantics rework).
...
- **ListItem vs Detail service models** — two tiers of denormalization for tasks. `TaskListItem` is a flat dataclass of Task fields plus resolved display names (`project_name`, `tag_names`) for list rendering without per-row lookups. `TaskDetail` is a flat dataclass of Task fields plus fully hydrated relationships (`status`, `project`, `group`, `edge_sources`, `edge_targets`, `history`, `tags`) for single-entity views. Both redeclare Task fields directly — they do not inherit from `Task`. `GroupRef` is the only surviving Ref type, used by `list_groups` to page through the hierarchy without hydrating every group. `WorkspaceListView` is the aggregate view model for `cmd_ls` — workspace + ordered statuses + TaskListItems. `WorkspaceContext` is the aggregate view model for `cmd_workspace_show` — `WorkspaceListView` + projects + tags + groups, for one-call AI session startup.
...
- **Journal / audit trail** — `_record_entity_changes()` in the service layer records `JournalEntry` rows for any entity mutation. `_record_edge_change()` handles edge link/unlink (task and group edges). Metadata mutations emit `meta.<key>` field entries via `_set_entity_meta` / `_remove_entity_meta` / `_replace_entity_metadata`. `EntityType` StrEnum defines the seven entity types (`task`, `project`, `group`, `workspace`, `status`, `task_edge`, `group_edge`). Per-entity field enums (`TaskField`, `ProjectField`, `GroupField`, `WorkspaceField`, `StatusField`, `EdgeField`) exist for documentation and validation but `journal.field` is an unconstrained TEXT column.
