...
- **Engine**: `src/stx/hooks.py` — 29 events in 5 payload categories (`created`, `updated`, `archived`, `meta`, `transferred`). Config path defaults to `~/.config/stx/hooks.toml`. Hooks are **post-only observers** — the write always proceeds; hooks observe committed state after the transaction. Post-hooks run via `subprocess.Popen` with `start_new_session=True`, stdout/stderr to DEVNULL, never raise back. `timing = "pre"` in hooks.toml raises a config error with a migration hint.
...
- **Service integration pattern** (`src/stx/service.py`):
  - **All mutations** — `fire_hooks(event, ...)` after the `with transaction` block commits. The hook always sees committed state.
  ...
  - **Edge metadata (composite PK)** — can't use the generic helpers. `set_edge_meta` / `remove_edge_meta` / `replace_edge_metadata` inline post-firing; `_edge_entity_snapshot` builds the `edgeEntity`-shaped dict and is re-called post-commit for fresh `version`.
- **Bulk-archive payload enrichment**: `cascade_archive_group`, `cascade_archive_workspace`, and the bulk paths of `archive_status` (`--force`, `--reassign-to`) emit only the top-level `*_ARCHIVED` event. Per-entity hooks for bulk-affected descendants are intentionally skipped. Instead, the event payload carries the affected ID lists: `archived_task_ids`, `archived_group_ids`, `archived_status_ids` (cascade ops), and `reassigned_task_ids` + `reassigned_to` (`archive_status --reassign-to`). These fields are optional — omitted when there are no bulk-affected entities. Hook authors who need to react to every affected entity read the ID lists from the parent event.
...
- **Bulk queries exclude description** — `_TASK_COLUMNS_NO_DESC` and `_GROUP_COLUMNS_NO_DESC` in `repository.py` list all columns except `description`, plus a computed `(description IS NOT NULL) AS has_description`. Bulk list functions (`list_tasks`, `list_tasks_filtered`, `list_tasks_by_status`, `list_tasks_by_ids`, `list_groups`, `list_groups_by_workspace`, `list_child_groups`, `get_group_ancestry`) use these constants. Detail functions (`get_task`, `get_group`, inserts, updates) still use `SELECT *`. `_resolve_description()` in `mappers.py` returns the real description when loaded, `DESCRIPTION_NOT_LOADED` sentinel (`"--description not loaded--"`) when `has_description` is truthy but the column is absent, or `None` when no description exists. Export uses `get_task_descriptions()` / `get_group_descriptions()` batch loaders to fetch descriptions separately. A schema-drift test in `test_mappers.py` asserts the column constants stay in sync with the schema.
...
- **`done` flag and `is_terminal`** — `task.done` is a sticky boolean set when a task enters a terminal status or via `mark_task_done`; cleared only by `mark_task_undone`. `status.is_terminal` controls the auto-flip (entry only — leaving a terminal status does not clear done). `compute_next_tasks` consults `task.done` only. Service models: `NextTasksView(workspace_id, ready: tuple[TaskListItem], blocked: tuple[BlockedTask])` and `BlockedTask(task: TaskListItem, blocked_by: tuple[int, ...])` — `blocked_by` is always non-empty (enforced by `__post_init__`).
