...
- **Journal / audit trail** — `_record_entity_changes()` in the service layer records `JournalEntry` rows for any entity mutation. `_record_edge_change()` handles edge link/unlink (task and group edges). Metadata mutations emit `meta.<key>` field entries via `_set_entity_meta` / `_remove_entity_meta` / `_replace_entity_metadata`. `EntityType` StrEnum defines six entity types (`task`, `group`, `workspace`, `status`, `task_edge`, `group_edge`). Per-entity field enums (`TaskField`, `GroupField`, `WorkspaceField`, `StatusField`, `EdgeField`) exist for documentation and validation but `journal.field` is an unconstrained TEXT column.
