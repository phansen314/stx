...
from stx.models import Status, Task
from stx.tui.markup import escape_markup
from stx.tui.model import WorkspaceModel
from stx.tui.widgets.task_card import TaskCard
...
class KanbanBoard(Horizontal):
    ...
    def _navigate(self, col_delta: int, row_delta: int) -> bool:
        ...
        # Row movement: clamp (no wrap); up-from-top focuses the column
        else:
            new_ci = ci
            if row_delta == -1 and ri == 0:
                col = self.query_one(f"#status-col-{focused.task_data.status_id}", KanbanColumn)
                self.screen.set_focus(col)
                return True
