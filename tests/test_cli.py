...
class TestTaskCommands:
    ...
    def test_show_with_deps(self, cli):
        ...
        assert "Edge targets: task:1" in out and "(blocks)" in out
        out2, _ = cli("task", "show", "1")
        assert "Edge sources: task:2" in out2 and "(blocks)" in out2
...
class TestJsonOutput:
    ...
    def test_transfer_dry_run(self, cli):
        ...
        cli("status", "create", "Inbox")
        data = self._json(
            cli, "task", "transfer", "1", "--to", "B2", "--status", "Inbox", "--dry-run"
        )
        assert data["ok"] is True
        payload = data["data"]
...
class TestErrorHandlingExtended:
    ...
    def test_keyboard_interrupt_exits_130(self, db_path, monkeypatch):
        ...
        monkeypatch.setitem(
            HANDLERS, "task_ls", lambda *a, **kw: (_ for _ in ()).throw(KeyboardInterrupt())
        )
...
class TestEditDryRun:
    ...
    def test_task_edit_dry_run_text(self):
        out, _ = self.cli(
            "task", "edit", "1", "--title", "T1 renamed", "--priority", "4", "--dry-run"
        )
        assert "dry-run" in out
...
class TestEndToEndSmoke:
    ...
    def test_show_and_detail(self):
        ...
        assert "task:1" in out  # edge target reference
    ...
    def test_dep_archive_and_recreate(self):
        ...
        self.cli("edge", "create", "--source", "2", "--target", "1", "--kind", "blocks")
        out, _ = self.cli("task", "show", "2")
        assert "Edge targets: task:1" in out  # blocked again
...
class TestTransferDryRunSourceName:
    ...
    def test_dry_run_shows_source_name_not_id(self):
        out, _ = self.cli(
            "task", "transfer", "1", "--to", "target-ws", "--status", "backlog", "--dry-run"
        )
        assert "dry-run" in out
