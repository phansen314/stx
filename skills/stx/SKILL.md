---
name: stx
description: Use when the user wants to persist structured context, a kanban board, or a multi-step plan across sessions via the local `stx` CLI — creating workspaces, managing projects and nested groups, tracking tasks, moving statuses, querying workspace state, workflow tracking, or running `stx workspace show`/`stx export`. Not for ad-hoc in-chat task decomposition.
---
...
4. **Check workspace state** at any point:
   ```sh
   stx workspace show   # full snapshot: statuses, tasks, projects, tags, groups
   stx task ls          # task list with filters (--project, --status, --tag, --search)
   ```

### Optional capabilities

- **Projects** — group related tasks: `stx project create "Q2 launch"`, then pass `--project "Q2 launch"` to `stx task create`.
...
- **Groups** — project-scoped hierarchies, nestable without depth limit: `stx group create "Sprint 1" --project "Q2 launch"` (`--project` is required), then `stx group assign <task> <group>` (or pass `--group/-g` at task create time to skip the separate assign step). Groups can be nested: `stx group create "Sub-group" --project "Q2 launch" --parent "Sprint 1"`.
- **Group edges** — `stx group edge create --source <title> --target <title> --kind blocks [--source-project <p>] [--target-project <p>]` (titles resolved within the active workspace; pass the project flags to disambiguate when titles collide or the edge is cross-project). Mirror commands for `archive`, `ls`, and `meta *`.
...
- **Audit trail** — `stx task log <task>` shows task field-change history. The underlying `journal` table covers all entity types (tasks, projects, groups, workspaces, statuses, task/group edges) and per-key metadata diffs — queryable directly via `stx export` or SQLite.
