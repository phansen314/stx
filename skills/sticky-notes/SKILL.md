...
1. **Pick or create a workspace.** Pass `--columns` to seed statuses — otherwise the workspace starts empty and `todo task create` will fail. Override active workspace per-command with `-w/--workspace`:
   ```sh
   todo workspace create work --columns "Backlog,In Progress,Done"
   ```

2. **Create tasks** — `-c/--column` is **required**, there is no default status:
   ```sh
...
- **Tags** — workspace-scoped, repeatable: `todo task create "..." -c Backlog --tag backend --tag ci`. Auto-created if missing.
...
- **Cross-workspace move** — use `todo task transfer` (not `todo task mv`): `todo task transfer task-0001 --workspace ops --column Backlog`.
