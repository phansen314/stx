...
- **Task metadata** — arbitrary key/value pairs on any task: `todo task meta set task-0001 branch feat/kv` then `todo task meta get task-0001 branch`. Keys are lowercase-normalized (`Branch` → `branch`), charset `[a-z0-9_.-]+`, max 64 chars. Values free-form up to 500 chars. Use for linking to external IDs (JIRA, GitHub, branches, PRs).
